...
Global Const BASS_CONFIG_ENCODE_QUEUE = &H10301
...
Global Const BASS_ENCODE_MONO = 256    ' convert to mono (if not already)
Global Const BASS_ENCODE_QUEUE = 512   ' queue data to feed encoder asynchronously
Global Const BASS_ENCODE_CAST_NOLIMIT = &H1000 ' don't limit casting data rate
Global Const BASS_ENCODE_LIMIT = &H2000 ' limit data rate to real-time
...
Global Const BASS_ENCODE_COUNT_QUEUE = 3 ' queued
Global Const BASS_ENCODE_COUNT_QUEUE_LIMIT = 4 '  queue limit
Global Const BASS_ENCODE_COUNT_QUEUE_FAIL = 5 ' failed to queue
...
Global Const BASS_ENCODE_NOTIFY_QUEUE_FULL = &H10001 ' queue is out of space

' BASS_Encode_ServerInit flags
Global Const BASS_ENCODE_SERVER_NOHTTP = 1  ' no HTTP headers
...
Declare Function BASS_Encode_ServerInit Lib "bassenc.dll" (ByVal handle As Long, ByVal port As String, ByVal buffer As Long, ByVal burst As Long, ByVal flags As Long, ByVal proc As Long, ByVal user As Long) As Long
Declare Function BASS_Encode_ServerKick Lib "bassenc.dll" (ByVal handle As Long, ByVal client As String) As Long
...
Function ENCODECLIENTPROC(ByVal handle As Long, ByVal connect As Long, ByVal client As Long, ByVal headers As Long, ByVal user As Long) As Long
    
    ' CALLBACK FUNCTION !!!

    ' Client connection notification callback function.
    ' handle : The encoder
    ' connect: BASSTRUE/BASSFALSE=client is connecting/disconnecting
    ...
    ' RETURN : BASSTRUE/BASSFALSE=accept/reject connection (ignored if connect=BASSFALSE)
